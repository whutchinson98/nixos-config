...
This is the source of truth for the Doom configuration. Doom's =:config literate= module tangles this file to =$DOOMDIR/config.el= on save and before =doom sync=.

=init.el= stays as a regular Doom file because it must enable the =literate= module before Doom can tangle this file. =packages.el= also stays as a regular Doom file, which is the common Doom setup.
...
Some Doom features use these for GPG configuration, email, file templates, and snippets.
...
Use Proton Mail Bridge locally. Bridge handles the Proton login; =mbsync= syncs
from Bridge into a local Maildir, =mu= indexes that Maildir, and =mu4e= reads it.
...
Match Alacritty's font configuration.
...
Vterm renders ANSI colors through its own faces (which inherit from =term-color-*=). Keep vterm aligned with the rest of the Gruvbox palette.
...
Set Org paths before Org loads.
...
Keep completed TODOs out of active agenda files by archiving them under =* Done= in =done.org= as soon as they enter a done state. The archive file stays out of =org-agenda-files= so it remains a history log instead of active work.
...
Discover projects when using project-switching commands.
...
** Rust workspaces in monorepos

In monorepos, the outer =.git= / =.jj= root hides nested Cargo workspaces from eglot, so rust-analyzer is handed the wrong cwd and reports "Failed to discover workspace". Register a =project-find-functions= entry that runs first and walks up looking for =Cargo.lock= (which only exists at workspace roots, never in member crates). This bypasses projectile / project-try-vc entirely for any file inside a Rust workspace.
