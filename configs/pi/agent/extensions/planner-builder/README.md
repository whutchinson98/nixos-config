...
| `/plan-build [--verify] [plan-file] [T01,T02]` | Run `builder` agents using the model and effort currently selected in the main pi process in monitored parallel Jujutsu workspaces when tasks are independent, require each completed task to create one atomic commit, and integrate commits serially onto the main workspace. Verifier review is skipped by default; pass `--verify` to run the `verifier` agent with the same selection and write `.pi/outputs/findings.html`. If no file is provided, the latest `.pi/plans/*.md` file is used. A live dashboard shows every launched agent, its current activity, and its streaming output; the builder watchdog periodically reports no-output status, cancels stuck attempts, and restarts them up to the configured limit. The result or failure is posted when complete. |
...
| `plan_file_build` | LLM-callable tool that runs builder agents with the model and effort selected in the main pi process against pending plan tasks in monitored parallel workspaces, cancels/restarts stuck builder attempts, and serially integrates atomic per-task `jj` commits. Set `runVerifier: true` to run the `verifier` agent with the same selection and write `.pi/outputs/findings.html`; the default is `false`. |
...
Running `/plan-build --verify` against an already completed plan skips its done tasks and triggers only the optional verifier review.
...
/plan-build --verify .pi/plans/20260507-180000Z-add-redis-caching-to-the-session-store.md
```
