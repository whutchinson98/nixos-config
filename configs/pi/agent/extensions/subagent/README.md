...
An agent can include `ask_user` in its frontmatter tool allowlist to pause and surface a material question through the parent pi UI. The subagent extension injects only the isolated question bridge into that subprocess, sends a system notification when a question is ready for the user, waits for the user's answer, and resumes the same agent run. In parallel mode, questions are presented and announced one at a time. Non-interactive runs tell the agent to proceed with and document a safe assumption.
